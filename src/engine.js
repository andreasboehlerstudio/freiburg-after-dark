...
import { createNightlifeCameos, updateNightlifeCameos, getNearbyCameo, interactNightlifeCameo } from './nightlife-cameos.js';
...
export class Game {
 ...
 start(heroId='nico',sidekickId='andreas',levelIndex=0) {
  ...
  this.heroes=[this.player,this.partner];this.humanCount=1;this.cooperative=false;
  this.heroes.forEach((h,index)=>{h.playerIndex=index;h.isHuman=index===0;});
  this.startSession(levelIndex);
 }
 startCoop(heroIds,levelIndex=0) {
  if(!Array.isArray(heroIds)||heroIds.length<2||heroIds.length>4||new Set(heroIds).size!==heroIds.length||heroIds.some(id=>!Object.hasOwn(HEROES,id)))throw new Error('Wähle zwei bis vier verschiedene Helden.');
  this.heroes=heroIds.map((id,index)=>({...this.makeHero(id,230,555),playerIndex:index,isHuman:true}));
  this.player=this.heroes[0];this.partner=this.heroes[1];this.humanCount=this.heroes.length;this.cooperative=true;
  this.startSession(levelIndex);
 }
 ...
 loadLevel(index) {
  ...
  this.levelIndex=index;this.level=LEVELS[index];this.camera=0;this.enemies=[];this.effects=[];this.pickups=[];this.rescues=this.cooperative?this.humanCount*2:2;this.transition=0;this.defeatTimer=0;this.shake=0;this.interactDown=false;
  ...
  this.cameos=createNightlifeCameos(this.level);
  this.residents=[];
  ...
  const starts=[[230,555],[145,610],[315,610],[145,475]];
  for(const [index,h] of this.heroes.entries()) {
   [h.x,h.y]=starts[index];
   ...
   h.heldItem=null;h.pendingInteract=null;h.interactDown=false;h.batInputUntil=0;h.airAttackUsed=false;h.landingRecovery=0;h.obstacleRoute=null;
  }
 ...
 waveEntries(arena,wave){
  ...
  // Advance through the roster across the whole district. Reusing only the
  // arena/wave indices would never schedule the tail of a larger local cast.
  const offset=this.level.waves.slice(0,arena).reduce((sum,entries)=>sum+entries.reduce((n,count)=>n+count,0),0)+waves.slice(0,wave-1).reduce((n,count)=>n+count,0)+this.levelIndex;
  const entries=Array.from({length:waves[wave-1]},(_,order)=>({order,boss:bossWave&&order===0,type:bossWave&&order===0?this.level.bossType:types[(offset+order)%types.length]}));
  if(this.cooperative){
   const roster=types.filter(type=>ENEMY_TYPES[type]&&!ENEMY_TYPES[type].passive&&ENEMY_TYPES[type].kind!=='passive');
   const active=entries.filter(entry=>!entry.boss&&!ENEMY_TYPES[entry.type]?.passive&&ENEMY_TYPES[entry.type]?.kind!=='passive').length;
   const extra=Math.ceil(active*(this.humanCount-1)/3);
   for(let i=0;i<extra&&roster.length;i++){const order=entries.length;entries.push({order,boss:false,type:roster[(offset+order)%roster.length]});}
  }
  return entries;
 }
 get maxActiveEnemies(){return this.cooperative?this.humanCount+3:MAX_ACTIVE_ENEMIES;}
 get maxEnemyAttackers(){return this.cooperative&&this.humanCount>=3?3:2;}
 get canAdvanceWave(){return this.cooperative?this.heroes.some(h=>h.hp>0):this.player.hp>0;}
 ...
 spawnEnemy({order,boss,type,resident=false,arenaIndex=this.arena?.index||0,wave=this.wave||1}) {
  ...
  const teamScale=this.cooperative?1+(this.humanCount-1)*(boss?.5:.1):1;
  const hp=d.hp*(boss?1:this.level.enemyHealthScale??1+Math.min(this.levelIndex,2)*.12)*teamScale;
 ...
 updateSpawnQueue(dt) {
  if(this.mode!=='playing'||!this.canAdvanceWave||!this.spawnQueue.length)return;
  if(this.enemies.filter(enemy=>enemy.hp>0).length>=this.maxActiveEnemies)return;
 ...
 pause(){if(this.mode==='playing'){this.shake=0;for(const h of this.heroes){h.pendingInteract=null;h.batInputUntil=0;}this.setMode('paused');}} resume(){if(this.mode==='paused')this.setMode('playing');}
 continueLevel(){
 ...
 getNearbyCameo(h=this.player){return getNearbyCameo(this,h);}
 interact(h) {
 ...
 updateProps(dt) {
  for(const prop of this.props){
   ...
   const source=this.heroes.find(h=>h.uid===prop.ownerUid),radius=prop.type==='bicycle'?48:30;
 ...
 attack(h,kind='light') {
  ...
  if(kind==='special'){h.energy-=45;h.specialCooldown=1.8;h.invuln=.85;this.stats.specials++;this.emit('special',{hero:h.heroId,x:h.x,y:h.y});if(h.heroId==='andreas')for(const ally of this.heroes){if(ally.hp>0){ally.hp=Math.min(ally.maxHp,ally.hp+25);this.effect('heal',ally.x,ally.y-75,{text:'+25',color:'#82ffe0',life:1,maxLife:1});}}}
 ...
 hurt(target,damage,source,knock=120) {
  if(target.type==='cameo')return false;
 ...
 heroUpdate(h,dt,inp) {
  ...
  const rescuing=(inp.revive||this.cooperative&&inp.interact)&&this.heroes.some(ally=>ally!==h&&ally.hp<=0&&distance(h,ally)<115);
  ...
  if(inp.interact&&!rescuing&&h.hitstun<=0&&h.z===0){const item=h.heldItem||this.getNearbyProp(h);if(item)h.pendingInteract={uid:item.uid,held:!!h.heldItem,until:this.time+.22};else if(interactNightlifeCameo(this,h)){h.pendingInteract=null;return;}}
 ...
 enemyUpdate(e,dt) {
  ...
  const alive=this.heroes.filter(h=>h.hp>0);if(!alive.length)return;
  const targetWeight=h=>distance(h,e)+(this.cooperative?this.enemies.filter(other=>other!==e&&other.attackTargetUid===h.uid&&(other.state==='telegraph'||other.state==='attack')).length*90:h.isPartner?0:-20);
  const target=alive.sort((a,b)=>targetWeight(a)-targetWeight(b))[0];
  ...
  if(Math.abs(dx)<e.range&&Math.abs(dy)<45&&e.cooldown<=0&&active<this.maxEnemyAttackers){
   ...
   e.enraged=e.isBoss&&e.hp<e.maxHp*.4;e.attackDuration=(e.telegraph+(e.attackKind==='slam'?.32:0))*(e.enraged?.8:1);e.attackTime=e.attackDuration;e.state='telegraph';e.attackTarget={x:target.x,y:target.y};e.attackTargetUid=target.uid;this.emit('telegraph',{boss:e.isBoss,kind:e.attackKind,x:e.x,y:e.y});return;
  }
  const hold=active>=this.maxEnemyAttackers&&Math.abs(dx)<160;const move=this.obstacleInput(e,target,{x:Math.abs(dx)>e.range*.78&&!hold?Math.sign(dx):0,y:Math.abs(dy)>22?Math.sign(dy):0});e.x+=move.x*e.speed*dt;e.y+=move.y*e.speed*.54*dt;e.state=move.x||move.y?'walk':'idle';
 }
 enemyStrike(e) {
  ...
  for(const h of this.heroes){if(e.attackHits.has(h.uid)||h.hp<=0)continue;const dx=h.x-e.x,dy=Math.abs(h.y-e.y);if(Math.abs(dx)<(slam?205:charge?100:e.range+20)&&dy<(slam?110:52)&&(slam||dx*e.facing>-28)){if(this.hurt(h,e.power*(slam?1.25:1),e,slam?300:180))e.attackHits.add(h.uid);}}
 }
 update(dt,input={}) {
  ...
  const inputs=this.cooperative?this.heroes.map((h,index)=>Array.isArray(input)?input[index]||{}:index===0?input:{}):[Array.isArray(input)?input[0]||{}:input];
  const humanInputs=inputs.map((raw,index)=>{const h=this.heroes[index],interactPressed=!!raw.interact&&!h.interactDown;h.interactDown=!!raw.interact;return {...raw,revive:raw.revive||this.cooperative&&raw.interact,interact:interactPressed};});
  this.interactDown=this.player.interactDown;
  const all=[...this.heroes,...this.attackableEnemies()];
  ...
  for(const [index,h] of this.heroes.entries())this.heroUpdate(h,dt,this.cooperative||index===0?humanInputs[index]:this.partnerInput(dt));
  for(const e of this.attackableEnemies())this.enemyUpdate(e,dt);
  ...
  if(this.cooperative)this.constrainTeam();
  for(const e of all){
  ...
  this.updateProps(dt);updateNightlifeCameos(this,dt);this.enemies=this.enemies.filter(e=>e.hp>0||e.deadTime>0);
  for(const fx of this.effects)fx.life-=dt;this.effects=this.effects.filter(f=>f.life>0);
  for(const item of this.pickups){item.life-=dt;for(const h of this.heroes){if(h.hp>0&&distance(h,item)<58){if(item.kind==='food')h.hp=Math.min(h.maxHp,h.hp+40);else h.energy=Math.min(100,h.energy+28);this.effect('heal',h.x,h.y-90,{text:item.kind==='food'?'+40 LP':'+28 EN',color:'#94ffcc',life:.9,maxLife:.9});item.life=0;this.emit('pickup',{kind:item.kind});break;}}}this.pickups=this.pickups.filter(p=>p.life>0);
  this.updateRescue(dt,this.cooperative?humanInputs:inputs[0]);
  if(this.mode!=='playing')return;
  if(this.waitingWave){if(this.canAdvanceWave)this.waveDelay-=dt;if(this.waveDelay<=0&&this.canAdvanceWave)this.spawnWave();}
  else this.updateSpawnQueue(dt);
  if(!this.waitingWave&&!this.spawnQueue.length&&!this.enemies.some(e=>e.hp>0)&&!this.arena.cleared){if(this.wave<this.level.waves[this.arena.index].length){this.wave++;this.waitingWave=true;this.waveDelay=1.4;}else{this.arena.cleared=true;this.announcement='WEITER →';this.announcementTime=3;this.score+=500;this.emit('arena-clear',{arena:this.arena.index});for(const h of this.heroes)if(h.hp>0)h.hp=Math.min(h.maxHp,h.hp+(this.level.arenaHeal||12));}}
  if(this.arena.cleared&&this.arena.index===this.level.waves.length-1&&!this.waitingWave&&!this.spawnQueue.length&&!this.enemies.some(enemy=>enemy.hp>0)&&this.heroes.some(h=>h.hp>0)){
  ...
  const advanceHeroes=this.cooperative?this.heroes.filter(h=>h.hp>0):[this.player];
  if(this.arena.cleared&&advanceHeroes.length&&advanceHeroes.every(h=>h.hp>0&&h.x>this.arena.right-95)&&this.arena.index<this.level.waves.length-1)this.enterArena(this.arena.index+1);
  if(this.cooperative)this.updateTeamCamera(dt);
  else{const desired=clamp(this.player.x-470,this.arena.left,Math.max(this.arena.left,this.arena.right-1280));const unlocked=clamp(this.player.x-470,0,this.level.width-1280);this.camera+=((this.arena.cleared?unlocked:clamp(desired,0,this.level.width-1280))-this.camera)*Math.min(1,dt*7);}
 }
 constrainTeam(){
  // Keep the entire team, including a fallen teammate, inside one shared view.
  // At full separation only outward travel stops; a stationary ally stays put.
  const margin=94,span=1280-margin*2;
  for(const h of this.heroes)h.x=clamp(h.x,margin,this.level.width-margin);
  let left=Math.min(...this.heroes.map(h=>h.x)),right=Math.max(...this.heroes.map(h=>h.x));
  if(right-left<=span)return;
  const oldLeft=Math.min(...this.heroes.map(h=>h.motionStartX)),oldRight=Math.max(...this.heroes.map(h=>h.motionStartX));
  const outwardLeft=Math.max(0,oldLeft-left),outwardRight=Math.max(0,right-oldRight),outward=outwardLeft+outwardRight;
  const excess=right-left-span;
  left+=excess*(outward>0?outwardLeft/outward:.5);right=left+span;
  for(const h of this.heroes)h.x=clamp(h.x,left,right);
 }
 updateTeamCamera(dt){
  const margin=94,left=Math.min(...this.heroes.map(h=>h.x)),right=Math.max(...this.heroes.map(h=>h.x));
  const target=clamp((left+right)/2-640,0,this.level.width-1280);
  this.camera+=(target-this.camera)*Math.min(1,dt*7);
  this.camera=clamp(this.camera,Math.max(0,right-1280+margin),Math.min(this.level.width-1280,Math.max(0,left-margin)));
 }
 updateRescue(dt,input){
  if(this.cooperative){
   const alive=this.heroes.filter(h=>h.hp>0);
   if(!alive.length){this.defeatTimer=(this.defeatTimer||0)+dt;if(this.defeatTimer>1.5){this.setMode('defeat');this.emit('defeat');}return;}
   this.defeatTimer=0;
   const helpers=new Set();
   for(const fallen of this.heroes.filter(h=>h.hp<=0)){
    const helper=alive.find(h=>!helpers.has(h.uid)&&input[h.playerIndex]?.revive&&distance(h,fallen)<115&&h.hitstun<=0&&h.attackTime<=0&&h.z===0&&h.state!=='dodge');
    if(helper){helpers.add(helper.uid);fallen.rescueProgress=(fallen.rescueProgress||0)+dt;}else fallen.rescueProgress=0;
    const manual=fallen.rescueProgress>=2.4,automatic=fallen.downTimer>=12&&this.rescues>0;
    if(!manual&&!automatic)continue;
    if(!manual)this.rescues--;
    fallen.hp=fallen.maxHp*.5;fallen.state='idle';fallen.invuln=3;fallen.hitstun=0;fallen.attackTime=0;fallen.pendingInteract=null;fallen.rescueProgress=0;fallen.downTimer=0;
    this.stats.revives++;this.emit('revive',{hero:fallen.heroId,automatic:!manual,rescues:this.rescues});
   }
   return;
  }
