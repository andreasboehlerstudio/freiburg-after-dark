...
export class StartTutorial {
  ...
  html({cooperative=false,bindings=[],touch=false}={}) {
    ...
    return `<section class="start-tutorial ${help?'has-keyboard-guide':''}" aria-labelledby="tutorial-title">
      ...
      <div class="tutorial-moves">${MOVES.map(([number, title, keys, pad], i) => `<article class="tutorial-move"><div class="tutorial-move-title"><span>${number}</span><h2>${title}</h2></div><div class="tutorial-key-row">${touch?['<kbd>LINKEN STICK ZIEHEN</kbd>','<kbd>SCHLAG</kbd>','<kbd>TRITT</kbd>'][i]:cooperative?i===0?'<kbd>WASD / PFEILE</kbd>':i===1?'<kbd>C / J</kbd>':'<kbd>V / K</kbd>':keys}<small>${touch?'TOUCH':pad}</small></div><canvas data-tutorial-pose="${i}" role="img" aria-label="${title.toLowerCase()}: dein gewählter Kämpfer"></canvas></article>`).join('')}</div>
      <div class="tutorial-extras">${(touch?[['SPRINGEN','SPRUNG','ANTIPPEN'],['AUSWEICHEN','AUSWEICHEN','ANTIPPEN'],['SPEZIAL','SPEZIAL','45 ENERGIE'],['AUFHEBEN / WERFEN','GREIFEN','HALTEN: AUFHELFEN']]:cooperative?[['SPRINGEN','LEER / ENTER','A'],['AUSWEICHEN','SHIFT LINKS / RECHTS','B'],['SPEZIAL','X / L','RB · 45 ENERGIE'],['AUFHEBEN / WERFEN','E / O','LB']]:[['SPRINGEN','LEERTASTE','A'],['AUSWEICHEN','SHIFT','B'],['SPEZIAL','L','RB · 45 ENERGIE'],['AUFHEBEN / WERFEN','E','LB']]).map(([title,key,pad])=>`<div><h3>${title}</h3><p><kbd>${key}</kbd><span>/ ${pad}</span></p></div>`).join('')}</div>
      ${help}
      <footer class="tutorial-footer"><div><p class="tutorial-tip">SPRUNGTRITT: IN DER LUFT ${touch?'SCHLAG ODER TRITT':cooperative?'C / V ODER J / K':'J ODER K'}.</p><p class="tutorial-team">DEIN SIDEKICK KÄMPFT AUTOMATISCH.</p><p class="tutorial-utilities"><button data-action="tutorial-back">← Zurück <span>ESC</span></button><span>${touch?'PAUSE / VOLLBILD: OBEN RECHTS':'IM SPIEL: ESC PAUSE · F VOLLBILD'}</span></p></div><button class="tutorial-start" data-action="tutorial-start">VERSTANDEN. LOS GEHT’S.<kbd>${touch?'ANTIPPEN':'ENTER / A'}</kbd></button></footer>
    </section>`;
  }
  begin(heroId, { cooperative = false, humanCount = 1, touch = false } = {}) {
    ...
    this.root.querySelector('.tutorial-team').textContent = cooperative
      ? `${humanCount} SPIELER · ZUSAMMENBLEIBEN · ${touch?'GREIFEN':'E / O / LB'} ZUM WIEDERBELEBEN HALTEN.`
