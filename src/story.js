...
/** Short chapter beats; art depicts the four friends, dialogue follows the selected team. */
export function storyForLevel(index,ids){
 const [first,second='Team',third=first,fourth=second]=names(ids),level=LEVELS[index];
 if(!level)return [];
 ...
 case 'park':return [scene(3,'Alle haben etwas gesehen','Der Kurier verschwindet am Kirchplatz. Ein Veranstalter, ein eifriger Fotograf und ein betrunkener Stammgast liefern drei Versionen derselben Nacht. Ein Name fällt immer: der Nachtbürgermeister.',[[third,'Ist der gewählt?'],[fourth,'Von seinen eigenen Türstehern.']],place)];
 case 'dreisam':return [scene(4,'Ein ruhiger Treffpunkt','Die Spur führt unter eine Brücke. Dort sollte die Tasche übergeben werden. Darin steckt die Abrechnung eines Clubbesitzers, der das ganze Nachtleben kontrollieren will. Der Nachtbürgermeister hat bereits Leute geschickt.',[[third,'Hier sollte doch niemand sein.'],[fourth,'Das war wohl die Beschreibung für nachher.']],place)];
 ...
 case 'bermuda':return [scene(6,'Die letzte Tür','Der Nachtkassierer hat geredet. Hinter der Mähne feiert der Nachtbürgermeister bereits seine „saubere, exklusive Innenstadt“. Ihr wollt das Fahrrad, die Abrechnung und Freiburgs Nacht zurück.',[['Die Mähne','Heute nur mit Gästeliste.'],[ids?.includes('andreas')?'Andreas':first,'Dann schreib uns drauf.']],place)];
 default:return [];
 }
}
