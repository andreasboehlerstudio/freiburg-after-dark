import {TouchControls,mergeTouch,touchMode} from './touch-controls.js';
...
import {CoopLobby,GamepadInput,connectedPads,coopKeyboardCommand,coopKeyboardHelp,COOP_KEYBOARD_CODES} from './local-coop-input.js';
...
let save={level:0,high:0,wins:0,hero:'andreas',partner:'nico'},settings={volume:.6,musicVolume:.5,effectsVolume:.8,voiceVolume:.75,music:true,touchControls:'auto',shake:!matchMedia('(prefers-reduced-motion: reduce)').matches};
...
settings.touchControls=touchMode(settings.touchControls);
const touch=new TouchControls({document,host:{navigator,matchMedia:query=>matchMedia(query),addEventListener:(...args)=>addEventListener(...args)},mode:settings.touchControls});
...
function drawCoopLobby(){
 ...
 setScreen('coop',`<section class="coop-screen" aria-labelledby="coop-title"><p class="eyebrow">COUCH-KOOP · LEVEL ${requestedLevel+1} / ${LEVELS.length}</p><h1 id="coop-title">Eine Straße. Euer Team.</h1><p class="subline">Jeder steuert seine eigene Figur. Verbindet zwei bis vier Spieler und wählt eure Helden.</p><div class="coop-roster">${Array.from({length:4},(_,index)=>{
  ...
  if(!slot)return `<article class="coop-slot"><div class="coop-slot-label">SPIELER ${index+1}</div><div class="coop-slot-empty"><strong>+</strong><span>Tastatur wählen oder Gamepad anschließen</span><kbd>A</kbd><span>ZUM BEITRETEN</span></div></article>`;
  return `<article class="coop-slot is-joined ${missingSlots.has(index)?'is-missing':''}"><div class="coop-slot-label">SPIELER ${index+1}<button class="coop-remove" data-action="coop-remove:${index}" aria-label="Spieler ${index+1} entfernen">×</button></div><canvas class="coop-portrait" data-hero="${slot.heroId}" role="img" aria-label="${HEROES[slot.heroId].name}"></canvas><div class="coop-hero-choice"><button class="coop-cycle" data-action="coop-prev:${index}" aria-label="Vorige Figur für Spieler ${index+1}">‹</button><strong>${HEROES[slot.heroId].name.split(' ')[0]}</strong><button class="coop-cycle" data-action="coop-next:${index}" aria-label="Nächste Figur für Spieler ${index+1}">›</button></div><p class="coop-controller">${slot.kind==='keyboard'?`TASTATUR ${(slot.keyboardIndex??0)===0?'LINKS':'RECHTS'}`:missingSlots.has(index)?`GAMEPAD ${slot.padIndex+1} FEHLT`:`GAMEPAD ${slot.padIndex+1} VERBUNDEN`}</p></article>`;
 }).join('')}</div><div class="coop-tools">${[0,1].map(key=>`<button class="coop-keyboard" data-action="${key===0?'coop-keyboard':'coop-keyboard-right'}" aria-pressed="${coopLobby.hasKeyboard(key)}" ${!coopLobby.hasKeyboard(key)&&count===4?'disabled':''}>${coopLobby.hasKeyboard(key)?'✓ ':'+ '}Tastatur ${key===0?'links · WASD':'rechts · Pfeile'}</button>`).join('')}<div class="coop-available">${count<4?available.map(entry=>`<button data-action="coop-add:${entry.index}">Gamepad ${entry.index+1} hinzufügen</button>`).join(''):''}</div></div><p class="coop-notice" role="status">${lobbyNotice|| (missing.length?'Ein zugewiesener Controller fehlt. Bitte wieder verbinden oder den Spieler entfernen.':'')}</p>${coopKeyboardHelp(coopLobby.slots)}<p class="coop-hint">A beitreten · ← / → Figur wählen · B verlassen · START losspielen. Mit Tastatur: Tab wählt Schaltflächen, Enter bestätigt. Bereits gewählte Figuren tauschen die Plätze.</p><footer class="coop-footer"><span class="coop-ready">${count} / 4 SPIELER ${ready?'· TEAM BEREIT':'· MINDESTENS 2'}</span><div class="actions"><button class="secondary" data-action="coop-back">Zurück</button><button class="primary" data-action="coop-start" ${ready?'':'disabled'}>Gemeinsam starten →</button></div></footer></section>`);
...
function lobbyAction(action){
 ...
 else if(kind==='coop-keyboard-right')coopLobby.setKeyboard(!coopLobby.hasKeyboard(1),1);
...
function showStartTutorial(snapshot){
 ...
 setScreen('tutorial',tutorial.html({...snapshot,touch:touch.enabled}));tutorial.begin(snapshot.hero,{cooperative:snapshot.cooperative,humanCount:snapshot.cooperative?snapshot.heroIds.length:1,bindings:snapshot.bindings,touch:touch.enabled});
...
function controls(){returnScreen=game.mode==='paused'?'pause':'menu';if(touch.enabled){setScreen('controls',`<div class="modal-backdrop"><section class="modal"><div class="eyebrow">HANDY · SPIELER 1</div><h1>Deine Daumen. Deine Straße.</h1><p class="subline">Links den Stick ziehen, rechts die beschrifteten Tasten antippen. Laufen und Angreifen funktionieren gleichzeitig.</p><div class="control-grid"><div>Schlag / Combo <kbd>SCHLAG halten</kbd></div><div>Schwerer Tritt <kbd>TRITT</kbd></div><div>Springen <kbd>SPRUNG</kbd></div><div>Ausweichen <kbd>AUSWEICHEN</kbd></div><div>Spezial · 45 Energie <kbd>SPEZIAL</kbd></div><div>Aufheben / Werfen <kbd>GREIFEN</kbd></div></div><p class="subline">Sprungtritt: SPRUNG, dann in der Luft SCHLAG oder TRITT. Mit einem Baseballschläger nutzt du SCHLAG. GREIFEN neben einem gefallenen Partner halten, um aufzuhelfen.</p><p class="small">Pause und Vollbild findest du oben rechts. Die Touch-Steuerung lässt sich unter Einstellungen automatisch, an oder aus schalten. Im Koop steuert sie die erste Figur.</p><div class="actions"><button class="primary" data-action="close-sub">Verstanden</button></div></section></div>`);return;}setScreen('controls',`<div class="modal-backdrop"><section class="modal"><div class="eyebrow">Tastatur & Standard-Gamepad</div><h1>Mach die Straße frei.</h1>${game.cooperative?coopKeyboardHelp(activeSession?.bindings):''}<div class="control-grid"><div>Bewegen <kbd>${game.cooperative?'Stick / Steuerkreuz':'WASD / ↑↓←→'}</kbd></div><div>Schlag / Combo <kbd>${game.cooperative?'Pad X':'J · Pad X'}</kbd></div><div>Schwerer Tritt <kbd>${game.cooperative?'Pad Y':'K · Pad Y'}</kbd></div><div>Springen <kbd>${game.cooperative?'Pad A':'LEER · Pad A'}</kbd></div><div>Ausweichen <kbd>${game.cooperative?'Pad B':'SHIFT · Pad B'}</kbd></div><div>Spezial (45 Energie) <kbd>${game.cooperative?'Pad RB':'L · Pad RB'}</kbd></div><div>Aufheben / Werfen <kbd>${game.cooperative?'Pad LB':'E · Pad LB'}</kbd></div><div>Pause / Vollbild <kbd>ESC / F</kbd></div></div><p class="subline">${game.cooperative?'Schlagt mit C / J / Pad X für eine dreiteilige Combo.':'J für eine dreiteilige Combo mehrfach drücken oder halten.'} ${game.cooperative?'In der Luft Schlag oder Tritt für einen Sprungtritt. Links E, rechts O oder Pad LB hebt Gegenstände auf und wirft sie. Mit einem Schläger benutzt ihr die Schlagtaste.':'In der Luft J oder K für einen gezielten Sprungtritt. E hebt Fahrräder oder Baseballschläger auf; mit E wirfst du sie. Mit einem Schläger schlägst du per J zu.'} Gegner müssen auf derselben Höhe und vor dir stehen. Rote Markierungen kündigen Angriffe an: ausweichen oder aus der Linie gehen.</p><p class="small">${game.cooperative?'Im lokalen Koop steuert jeder seine eigene Figur. Neben einem gefallenen Mitspieler links E, rechts O oder am Gamepad LB halten, um ihn aufzuhelfen. Fällt das ganze Team, endet der Versuch.':'Dein Sidekick kämpft automatisch. Bei ihm E halten hilft schneller; sonst steht er nach 12 Sekunden auf. Er kann dich zweimal pro Level retten. Sind beide am Boden, endet der Versuch.'} Essen und Energiedrinks werden beim Darüberlaufen eingesammelt.</p><div class="actions"><button class="primary" data-action="close-sub">Verstanden</button></div></section></div>`)}
function options(){returnScreen=game.mode==='paused'?'pause':'menu';setScreen('settings',`<div class="modal-backdrop"><section class="modal"><div class="eyebrow">Deine Nacht, dein Sound</div><h1>Einstellungen</h1><label class="setting">Lautstärke<input id="volume" aria-label="Lautstärke" type="range" min="0" max="100" value="${Math.round(settings.volume*100)}"></label>${[["musicVolume","Musik"],["effectsVolume","Treffer & Effekte"],["voiceVolume","Kampflaute"]].map(([id,label])=>`<label class="setting">${label}<input id="${id}" aria-label="${label}" type="range" min="0" max="100" value="${Math.round(settings[id]*100)}"></label>`).join('')}<label class="setting">Musik an<input id="music" type="checkbox" ${settings.music?'checked':''}></label><label class="setting">Kamerawackeln bei Treffern<input id="shake" type="checkbox" ${settings.shake?'checked':''}></label><label class="setting">Touch-Steuerung<select id="touch-mode" aria-label="Touch-Steuerung">${[['auto','Automatisch'],['on','An'],['off','Aus']].map(([value,label])=>`<option value="${value}" ${settings.touchControls===value?'selected':''}>${label}</option>`).join('')}</select></label><p class="small">Touch steuert Spieler 1. Links ziehen, rechts Aktionen antippen. GREIFEN hebt auf und wirft; zum Aufhelfen halten. Querformat bietet mehr Platz.</p><p class="small">Lokaler Koop: zwei Spieler an einer Tastatur, insgesamt bis zu vier mit zusätzlichen Gamepads. Links: WASD, C/V/X, Leertaste, E, linke Shift. Rechts: Pfeile, J/K/L, Enter, O, rechte Shift. Auflösung folgt Fenster und Bildschirm bis 3840 × 2160. F schaltet Vollbild. Bei Fokusverlust pausiert das Spiel automatisch. Einstellungen und freigespielte Level werden auf diesem Gerät gespeichert.</p><div class="actions"><button class="primary" data-action="close-sub">Fertig</button><button class="secondary" data-action="fullscreen">Vollbild</button></div></section></div>`);document.querySelector('#touch-mode').onchange=e=>{settings.touchControls=touchMode(e.target.value);touch.setMode(settings.touchControls);persist()};document.querySelector('#volume').oninput=e=>{settings.volume=Number(e.target.value)/100;audio.setVolume(settings.volume);persist()};for(const id of ['musicVolume','effectsVolume','voiceVolume'])document.querySelector('#'+id).oninput=e=>{settings[id]=Number(e.target.value)/100;audio.setMix({music:settings.musicVolume,effects:settings.effectsVolume,voice:settings.voiceVolume});persist()};for(const id of ['music','shake'])document.querySelector('#'+id).onchange=e=>{settings[id]=e.target.checked;audio.setMusic(settings.music);persist()}}
...
async function fullscreen(){try{if(document.fullscreenElement)await document.exitFullscreen();else if(document.documentElement.requestFullscreen)await document.documentElement.requestFullscreen();else document.querySelector('#status').textContent='Für Vollbild in Safari: Teilen, Zum Home-Bildschirm, Als Web-App öffnen.';}catch{document.querySelector('#status').textContent='Vollbild ist in diesem Browser gerade nicht verfügbar.';}}
...
const handled=[...COOP_KEYBOARD_CODES,'KeyW','KeyA','KeyS','KeyD','ArrowUp','ArrowDown','ArrowLeft','ArrowRight','KeyJ','KeyK','KeyL','KeyE','Space','ShiftLeft','ShiftRight'];
addEventListener('keydown',e=>{heldKeys.add(e.code);inputSeen[e.code]=(inputSeen[e.code]||0)+1;
 ...
 if(screen==='coop'){
  ...
  if(!(isDev&&['F1','F2','F3','F4','F5','F6','F11'].includes(e.code))){
   ...
   if((handled.includes(e.code)&&e.code!=='Space'&&e.code!=='Enter')||/^F\d+$/.test(e.code))e.preventDefault();
   return;
  }
 }
...
function input(){
 ...
 if(screen==='playing'){
  ...
  if(activeSession?.cooperative)return activeSession.bindings.map((slot,index)=>{const command=slot.kind==='keyboard'?coopKeyboardCommand(has,slot.keyboardIndex??0,{shared:activeSession.bindings.some(binding=>binding.kind==='keyboard'&&binding.keyboardIndex===1)}):padInput.command(slot.padIndex);return index===0?mergeTouch(command,touch.command()):command;});
  if(!pad)return mergeTouch(keyboard,touch.command());
  ...
  keyboard.x=cmd.x||keyboard.x;keyboard.y=cmd.y||keyboard.y;return mergeTouch(keyboard,touch.command());
 }
...
function tick(now){
 ...
 renderer.inputBindings=activeSession?.bindings;
