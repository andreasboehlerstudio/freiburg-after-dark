...
function showStartTutorial(snapshot){
 ...
 setScreen('tutorial',tutorial.html(snapshot));tutorial.begin(snapshot.hero,{cooperative:snapshot.cooperative,humanCount:snapshot.cooperative?snapshot.heroIds.length:1,bindings:snapshot.bindings});
...
function input(){
 ...
 if(screen==='playing'){
  ...
  if(activeSession?.cooperative)return activeSession.bindings.map(slot=>slot.kind==='keyboard'?coopKeyboardCommand(has,slot.keyboardIndex??0,{shared:activeSession.bindings.some(binding=>binding.kind==='keyboard'&&binding.keyboardIndex===1)}):padInput.command(slot.padIndex));
